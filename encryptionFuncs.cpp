...
uint64_t processBlock(uint64_t block, bitset<KEYSIZE> key, uint16_t subkeyVals[][12]) {
...
int main(int argc, char *argv[]) {
    ...
    makeSubkeys(&testKey, subkeyVals, decSubkeyVals);

    uint64_t block = stoull(plaintext, nullptr, 16);
    cout << "cipher block: " << hex << processBlock(block,testKey, subkeyVals) << endl;
    /*
